...
fn read_from_disk(filename: String) -> String {
    // get a string from JS, error if it is not there
    // let filename = duk::require_string(ctx, 0);
    ...
    let real_path = match fs::canonicalize(path) {
        ...
        Err(err) => {
            ...
            //     ErrorKind::NotFound => {
                ...
                //     let args = panic!("Failed to canonicalize {} - {:?}", filename, err.kind());
                //     // duk::error(ctx, _DUK_ERR_ERROR, args);
                //     // return 0;
                //     return " ".to_owned();
                // }
    ...
    let mut file = match File::open(real_path) {
        ...
        Err(err) => {
            ...
            // match err.kind() {
            //     _ => {
                    /*let args = */panic!("Failed to open {} - {:?}", filename, err.kind());
                    // duk::error(ctx, _DUK_ERR_ERROR, args);
                    // return 0;
            //     }
            // }
